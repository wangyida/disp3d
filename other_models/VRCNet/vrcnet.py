...
from other_models.VRCNet.models.pcn import PCN_encoder
...
class MSAP_SKN_decoder(nn.Module):
    ...
    def forward(self, global_feat, point_input):
        ...
        # Here is changing the coarse output
        """
        coarse_raw = self.gcn_dec([global_feat.unsqueeze(1)]).transpose(1, 2)
        """
...
class Model(nn.Module):
    def __init__(self, args, size_z=128, global_feature_size=1024):
        ...
        self.encoder = gcn3d.GCN3DFeatDeep(support_num=10, neighbor_num=10)
        """
    ...
    def forward(self, x, mean_feature=None, alpha=None):
        num_input = x.size()[2]

        points = x
        feat = self.encoder(points)
        # feat = self.encoder(points).view(x.shape[0], -1)
        ...
        # return {'out1': coarse_raw, 'out2': fine, 'out3': coarse_high, 'out4': coarse}
        return coarse_raw, fine, coarse_high, coarse

    def trainer(self, x, gt, mean_feature=None, alpha=None):
        num_input = x.size()[2]
        ...
        feat = self.encoder(points)
        # feat = self.encoder(points.transpose(1, 2)).view(x.shape[0], -1)
    ...
    def evaluate(self, x, gt, mean_feature=None, alpha=None):
        num_input = x.size()[2]
        ...
        return {'emd': emd, 'cd_p': cd_p, 'cd_t': cd_t, 'f1': f1}
