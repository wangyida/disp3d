...
from softpool import Periodics
from FNet import FNet
from activations.siren import Siren
...
class Conv_surface(nn.Module):
    ...
    def __init__(self, kernel_num, support_num):
        ...
        self.directions = nn.Parameter(
            torch.FloatTensor(128, support_num * kernel_num))
        self.initialize()
    ...
    def forward(self, neighbor_index: "(bs, vertice_num, neighbor_num)",
        ...
        theta = torch.max(
            theta, dim=2)[0] * self.weights # (bs, vertice_num, support_num, kernel_num)
...
class Conv_layer(nn.Module):
    def __init__(self, in_channel, out_channel, support_num):
        ...
        self.directions = nn.Parameter(
            torch.FloatTensor(32, support_num * out_channel))
        self.initialize()
...
class GCN3DFeatDeep(nn.Module):
    def __init__(self, support_num: int, neighbor_num: int):
        ...
        self.conv_5 = Conv_layer(512, 512, support_num=support_num)
        self.pool_3 = Pool_layer(pooling_rate=8, neighbor_num=8)
        self.conv_6 = Conv_layer(512, 1024, support_num=support_num)
    ...
    def forward(self, vertices: "(bs, vertice_num, 3)"):
        ...
        # feature_global = fm_6[:,:2,:].max(1, keepdim=True)[0]
