from pointcnn.Utils import knn_indices_func_cpu
import torch
from torch.autograd import Variable
import numpy as np
import torch.nn as nn
from torch import cuda, FloatTensor, LongTensor
from pointcnn.xConv import XConv, Dense
...
class PointCnnLayer(nn.Module):
    def __init__(self, features, settings):
        ...
        self.xvonc1 = XConv(
            ...
            P=self.settings[0][0].get('P'),
            C_mid=settings[0][0].get('C') // 2,
            depth_multiplier=4)  ## First XConvolution

        self.dense1 = Dense(
            settings[0][0].get('C'), settings[0][1].get('C') // 2, drop=0)
        self.xvonc2 = XConv(
            C_in=settings[0][1].get('C') // 2,
            C_out=settings[0][1].get('C'),
            dims=3,
            K=settings[0][1].get('K'),
            P=self.settings[0][1].get('P'),
            C_mid=settings[0][1].get('C') // 4,
            depth_multiplier=settings[0][0].get('C') //
            4)  ## Second XConvolution

        self.dense2 = Dense(
            settings[0][1].get('C'), settings[0][2].get('C') // 2, drop=0)
        self.xvonc3 = XConv(
            C_in=settings[0][2].get('C') // 2,
            C_out=settings[0][2].get('C'),
            dims=3,
            K=settings[0][2].get('K'),
            P=self.settings[0][2].get('P'),
            C_mid=settings[0][2].get('C') // 4,
            depth_multiplier=settings[0][1].get('C') //
            4)  ## Third XConvolution

        self.dense3 = Dense(
            settings[0][2].get('C'), settings[0][3].get('C') // 2, drop=0)
        self.xvonc4 = XConv(
            C_in=settings[0][3].get('C') // 2,
            C_out=settings[0][3].get('C'),
            dims=3,
            K=settings[0][3].get('K'),
            P=self.settings[0][3].get('P'),
            C_mid=settings[0][3].get('C') // 4,
            depth_multiplier=settings[0][2].get('C') //
            4)  ## Third XConvolution
        ...
        for i in range(0, len(settings[1])):
            deconvolutions["deconv" + str(i)] = XConv(
                C_in=self.layers_conv[settings[1][i].get('pts_layer_idx')].C,
                ...
                C_mid=self.layers_conv[settings[1][i].get('pts_layer_idx')].C
                // 4,
                depth_multiplier=1)  ## First dXConvolution

            dense_deconv["dense" + str(i)] = Dense(
                self.layers_conv[settings[1][i].get('qrs_layer_idx')].C * 2,
                self.layers_conv[settings[1][i].get('qrs_layer_idx')].C)
        ...
        for i in range(0, len(settings[2])):
            fc["fc" + str(i)] = Dense(
                self.dense_deconv[-1].out
                if i == 0 else fc["fc" + str(i - 1)].out,
                settings[2][i].get('C'),
    ...
    def forward(self, x):
        ...
        if (self.settings[0][0].get('P') != -1):
            idxx = np.random.choice(
                x.size()[1], self.settings[0][0].get('P'),
                replace=False).tolist()  ## select representative points
            rep_pts = x[:, idxx, :]
        else:
            rep_pts = x
        pts_idx = knn_indices_func_cpu(rep_pts, x,
                                       self.settings[0][0].get('K'),
                                       self.settings[0][0].get('D'))
        #pts_idx = pts_idx[:,::self.settings[0][0].get('D'),:]
        pts_regional = torch.stack([
            x[n][idx, :] for n, idx in enumerate(torch.unbind(pts_idx, dim=0))
        ],
                                   dim=0)
        ...
        if (not (self.settings[0][1].get('P') == -1)):
            ...
            idxx = np.random.choice(
                rep_pts.size()[1], self.settings[0][1].get('P'),
                replace=False).tolist()  ## select representative points
            rep_pts2 = rep_pts[:, idxx, :]
        else:
            rep_pts2 = rep_pts
        fts = self.dense1(out)
        pts_idx = knn_indices_func_cpu(rep_pts2, rep_pts,
                                       self.settings[0][1].get('K'),
                                       self.settings[0][1].get('D'))
        #pts_idx = pts_idx[:,:,::self.settings[0][1].get('D')]
        pts_regional = torch.stack([
            rep_pts[n][idx, :]
            for n, idx in enumerate(torch.unbind(pts_idx, dim=0))
        ],
                                   dim=0)
        fts_regional = torch.stack([
            fts[n][idx, :]
            for n, idx in enumerate(torch.unbind(pts_idx, dim=0))
        ],
                                   dim=0)
        ...
        if (not (self.settings[0][2].get('P') == -1)):
            ...
            idxx = np.random.choice(
                rep_pts2.size()[1],
                self.settings[0][2].get('P'),
                replace=False).tolist()  ## select representative points
            rep_pts3 = rep_pts2[:, idxx, :]
        else:
            rep_pts3 = rep_pts2
        fts = self.dense2(out2)
        pts_idx = knn_indices_func_cpu(rep_pts3, rep_pts2,
                                       self.settings[0][2].get('K'),
                                       self.settings[0][2].get('D'))
        ...
        pts_idx = knn_indices_func_cpu(rep_pts4, rep_pts3,
                                       self.settings[0][3].get('K'),
                                       self.settings[0][3].get('D'))
        ...
        for i in range(0, len(self.deconvolutions)):
            ...
            this_out = outs[self.settings[1][i].get('pts_layer_idx') +
                            1] if i == 0 else outs[-1]
            ...
            pts_idx = knn_indices_func_cpu(rep, rep2,
            ...
            out = torch.cat(
                (out, outs[self.settings[1][i].get('qrs_layer_idx') + 1]), -1)
...
if __name__ == "__main__":
    x = 8
    xyz = torch.rand(4, 2048, 3).cuda()
    xconv_param_name = ('K', 'D', 'P', 'C')
    xconv_params = [
        dict(zip(xconv_param_name, xconv_param))
        for xconv_param in [(8, 1, -1, 256), (12, 2, 768,
                                              256), (16, 2, 384,
                                                     512), (16, 4, 128, 1024)]
    ]
    ...
    xdconv_params = [
        ...
        for xdconv_param in [(16, 4, 3, 3), (16, 2, 3, 2), (12, 2, 2,
                                                            1), (8, 2, 1, 0)]
    ]
    ...
    model = PointCnnLayer(xyz, ["features"],
                          [xconv_params, xdconv_params, fc_params]).cuda()
    out = model(xyz)
    print(out.shape)
