...
class PoinTr(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        if self.refiner == 'foldingnet':
            self.foldingnet = Fold(self.trans_dim, step = self.fold_step, hidden_dim = 256)  # rebuild a cluster point
        elif self.refiner == 'pointgcn':
            from GCN.gcn_decode import Generator
            self.gcn_dec = Generator(
                features=[384, 256, 64, 3],
                degrees=[1, 2, 2],
                support=10,
                root_num=1)
        elif self.refiner == 'renet':
            ### NOTE: Yida
            self.num_coarse_raw = self.num_query*2 
            self.num_fps = 2048
            self.num_coarse = 2048
            self.num_fine = self.num_pred 
            self.points_label = True
            self.local_folding = True
            ...
            self.encoder = SA_SKN_Res_encoder(input_size=self.input_size, k=[16], pk=10,
                                              output_size=self.dense_feature_size, layers=[1,1,1,1])

            self.up_scale = int(np.ceil(self.num_fps / (self.num_coarse_raw)))
    ...
    def forward(self, xyz):
        ...
        if self.refiner == 'foldingnet':
            ...
            ret = (querries_all, rebuild_points, rebuild_points)
        elif self.refiner == 'renet':
            # NOTE: Yida
            # NOTE: following codes are VRCNet's sampling strategy
            querries_all = querries_all.transpose(1, 2)
            coarse_point_cloud = coarse_point_cloud.transpose(1, 2)
            inp_sparse = inp_sparse.transpose(1, 2)
            if self.points_label:
                id0 = torch.zeros(coarse_point_cloud.shape[0], 1, coarse_point_cloud.shape[2]).cuda().contiguous()
                coarse_input = torch.cat( (coarse_point_cloud, id0), 1)
                id1 = torch.ones(inp_sparse.shape[0], 1, inp_sparse.shape[2]).cuda().contiguous()
                org_points_input = torch.cat( (inp_sparse, id1), 1)
            else:
                coarse_input = coarse_point_cloud
            ...
            if coarse_fps.size()[2] > self.num_coarse:
                ...
                idx_scores = scores.topk(k=self.num_coarse, dim=2)[1].view(B, -1).int()
            ...
            ret = (querries_all.transpose(1, 2), fine.transpose(1, 2), coarse_fps.transpose(1, 2))
        return ret
