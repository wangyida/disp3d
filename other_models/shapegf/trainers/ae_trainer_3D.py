...
import importlib
import numpy as np
from shapegf.trainers.base_trainer import BaseTrainer
from shapegf.trainers.utils.vis_utils import visualize_point_clouds_3d, \
    visualize_procedure
from shapegf.trainers.utils.utils import get_opt, get_prior, \
...
class Trainer(BaseTrainer):
    ...
    def update(self, data, data_gt, **kwargs):
        ...
        tr_pts = data.cuda()  # (B, #points, 3)smn_ae_trainer.py
        tr_gt_pts = data_gt.cuda()  # (B, #points, 3)smn_ae_trainer.py
        ...
        res = score_matching_loss(self.score_net, z, tr_gt_pts, used_sigmas)
        loss = res['loss']
        """
        if not no_update:
            loss.backward()
    ...
    def validate(self, test_loader, epoch, **kwargs):
