...
class Decoder(nn.Module):
    ...
    def forward(self, feat, partial=[], return_P0=False):
        ...
        if partial != []:
            ...
            pcd = fps_subsample(pcd, self.num_p0)
        if return_P0:
            arr_pcd.append(pcd)
        K_prev = None
...
class SnowflakeNet(nn.Module):
    def __init__(self, dim_feat=512, num_pc=256, num_p0=512, radius=1, up_factors=None, global_feat=False):
        """
        Args:
        ...
        # NOTE: just one type of input
        # self.decoder = Decoder(dim_feat=dim_feat, num_pc=num_pc, num_p0=num_p0, radius=radius, up_factors=up_factors)
        # NOTE: dim_feat=dim_feat*2 if image and point cloud are fed in the same time
        if global_feat == False:
            self.decoder = Decoder(dim_feat=dim_feat, num_pc=num_pc, num_p0=num_p0, radius=radius, up_factors=up_factors)
        elif global_feat == True:
            self.decoder = Decoder(dim_feat=dim_feat*2, num_pc=num_pc, num_p0=num_p0, radius=radius, up_factors=up_factors)

    # NOTE: original codes
    # def forward(self, point_cloud, return_P0=False):
    def forward(self, point_cloud=[], global_feature=[], return_P0=False):
        """
        Args:
            point_cloud: (B, N, 3)
        """
        if point_cloud != [] and global_feature != []:
            pcd_bnc = point_cloud
            point_cloud = point_cloud.permute(0, 2, 1).contiguous()
            feat = self.feat_extractor(point_cloud)
            feat = torch.cat((feat, global_feature), 1)
            ...
            feat = global_feature
            out = self.decoder(feat, return_P0=return_P0)
        return out
